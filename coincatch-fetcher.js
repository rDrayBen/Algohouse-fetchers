...
async function FUTURES(){
    // define the websocket and REST URLs
    const wsUrl = 'wss://stream.coincatch.com/mix/v1/stream?compress=false&terminalType=1';
    const restUrl = "https://www.coincatch.com/v1/mcp/contract/list2";
    ...
    // extract symbols from JSON returned information
    for(let i = 0; i < myJson1['data']['listDTOS'][0]['symbolDTOList'].length; ++i){
        currencies.push(myJson1['data']['listDTOS'][0]['symbolDTOList'][i]['symbolDisplayName']);
    }

    // print metadata about pairs
    async function Metadata(){
        myJson1['data']['listDTOS'][0]['symbolDTOList'].forEach((item)=>{
