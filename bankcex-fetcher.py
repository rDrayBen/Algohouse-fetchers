...
currency_url = 'https://bankcex.com/exchange/getProducts'
answer = requests.get(currency_url)
currencies = answer.json()
list_currencies = list()
WS_URL = 'wss://socket.bankcex.com/socket.io/?EIO=3&transport=websocket'
...
def get_order_books(var):
	...
	if 'a' in order_data and len(order_data["a"]) != 0:
		...
		pq = "|".join(str(el[1]) + "@" + str("{:.8f}".format(float(el[0]))) for el in order_data["a"])
	...
	if 'b' in order_data and len(order_data["b"]) != 0:
		...
		pq = "|".join(str(el[1]) + "@" + str("{:.8f}".format(float(el[0]))) for el in order_data["b"])
		answer = order_answer + pq
